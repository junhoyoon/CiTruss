...
double ThetaActiveSet(
  ...
  fprintf(stdout, "%ld %ld %ld %ld\n", X.size(), X[0].size(), Y.size(), Y[0].size());
	...
	for (long i = 0; i < p; i++) {
        ...
        double tmp = fabs(L1SubGrad(Theta_ij, G_ij, lambda_x));
        if (std::isnan(tmp)) {
          fprintf(stdout, "%ld %ld %f %f %f %ld %ld %ld\n", i, j, Theta_ij, G_ij, lambda_x,n_x,n_y,n_o);
...
void mega_scggm(
	...
	if (!options.quiet) {
		fprintf(stdout, "min_blocks_Theta:%ld min_blocks_Lambda:%ld \n", 
  ...
  auto time_begin = chrono::high_resolution_clock::now();
    ...
    double time_elapsed = chrono::duration<double>(time_end - time_begin).count();
  	fprintf(stdout, "Theta active: %f sec.\n", time_elapsed);
    ...
    time_elapsed = chrono::duration<double>(time_end - time_begin).count();
  	fprintf(stdout, "Lambda active: %f sec.\n", time_elapsed);
    ...
    time_end = chrono::high_resolution_clock::now();
    time_elapsed = chrono::duration<double>(time_end - time_begin).count();
  	fprintf(stdout, "Graph clustering: %f sec.\n", time_elapsed);
    time_begin = chrono::high_resolution_clock::now();
    ...
    time_end = chrono::high_resolution_clock::now();
    time_elapsed = chrono::duration<double>(time_end - time_begin).count();
  	fprintf(stdout, "Lambda CD: %f sec.\n", time_elapsed);
    time_begin = chrono::high_resolution_clock::now();
    ...
    time_end = chrono::high_resolution_clock::now();
    time_elapsed = chrono::duration<double>(time_end - time_begin).count();
  	fprintf(stdout, "Objective: %f sec.\n", time_elapsed);
